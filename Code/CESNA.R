...
if(sim == TRUE){
  ...
  ## setting alpha value
  nc <- c(3)#4
  alpha <- c(0.0005, 0.0008,0.001)#c(0.001)#c(0.0002,0.0005,0.0008,0.001)#0.0002
  alphaLL <- c(1)#NULL#c(0.5)#c(1)#c(0.7)#c(0, 0.3, 0.5, 0.7, 0.9, 1)#c(0.2,0.5,0.7)#0.5
  lambda <- 0.001
  thresh <- 0.0001
  randomize = TRUE
  missing = 0
      ...
      for (a2 in alpha) {
        for(j in 1:Nsim){
          ...
          op <- CESNA(G,ncVal,k,o,N,a2,lambda,
                      ...
                      orig,CovNamesLin,CovNamesLP,NULL)
          opf[[lvl]] <- op
          lvl <- lvl+1
        }
      }
