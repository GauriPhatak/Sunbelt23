...
mem_specCov <- CASC(adj_m, as.numeric(cov[-match(todel, cov$name),4]), 
                      K = 5,  itermax  = 1000, startn = 50, alphan = 1)
png("spectralCov.png", 600,500)
plot_graph(delete_vertices(WWgrph, todel), mem_specCov, NA)
dev.off()
...
plt <- grid.arrange(p$Edge_betweenness,  p$Spectral, p$SpectralCov, ncol =1)
...
compare(op$Edge_betweenness[!is.na(op$SpectralCov)], 
        op$Spectral[!is.na(op$SpectralCov)], 
        method = "adjusted.rand")

## Comparing edge betweenness and spectral w covariates
compare(op$Edge_betweenness[!is.na(op$SpectralCov)], 
        op$SpectralCov[!is.na(op$SpectralCov)], 
        method = "adjusted.rand")

## comparing between spectral w and w/o covariates

compare(op$Spectral[!is.na(op$SpectralCov)], 
        op$SpectralCov[!is.na(op$SpectralCov)], 
        method = "adjusted.rand")
