...
V(g)$name <- as.character(1:length(V(g)))
adj_m <- as_adjacency_matrix(g, sparse = FALSE)
...
## Regularized Spectral  clustering 
mem_spec <- RegSpectralClust(g, k, FALSE)
png("spectral.png", 600,500)
plot_graph(g, mem_spec, NA)
dev.off()

mem_edge <- cluster_edge_betweenness(g, weights = 1/(E(g)$distance),directed = FALSE)
png("edgebet.png", 600,500)
plot_graph(g, membership(mem_edge), NA)
dev.off()
...
op<- as.data.frame(cbind(Edge_betweenness = membership(mem_edge),
           ...
           SpectralWCovariates = mem_specCov))
p  <- BOXPLOTS(op,V(g)$delta, g, 3, 6)
aov <- p[[2]]
p <- p[[1]]
