...
param <- "InitParamMiss_Cohesive_MCAR_B15_LASSO.rds"
param_c <- "InitParamMiss_Cohesive_MCAR_B15_LASSO_comb.rds"
fileName <- list.files("CoDAOP/CohesiveMCAR_B15_LASSO/")
#Ncombo <- c(1:144)#c(1,2, 4:46, 48:72, 74:144)
S <- as.data.frame(readRDS(param_c))

S$missing <- rep(c(0,5,10,25,45), each =2)

df_op <- data.frame(matrix(ncol = 10,nrow = 0,0))
#idx <- c()
...
getData <- function(df){
  ...
  return(cbind(lens,OI,ARI,Acc1,Acc2,Acc3,iter))
}


for(i in fileName){
  idx <- str_extract_all(i, "\\d+")[[1]][2]
  df <- readRDS(paste0("CoDAOP/CohesiveMCAR_B15_LASSO/",i))
  df_reg <- df[[1]]
  df_sto <- df[[3]]
  df_nocov <- df[[2]]
