...
fileName <- list.files("CoDAOP/CohesiveMCAR_2_NoDiag/")#"CoDAOP/CohesiveMCAR_B15_LASSO/")
#Ncombo <- c(1:144)#c(1,2, 4:46, 48:72, 74:144)
S <- as.data.frame(readRDS(param_c))

S$missing <- rep(c(0,5,10,25,45), each =9)

df_op <- data.frame(matrix(ncol = 14,nrow = 0,0))
#idx <- c()

getData <- function(df){
  lens <- unlist(lapply(df, function(x ) length(x$Loglik)))
  ...
  nsim <- rep(1:5, each = 5*2)
  return(cbind(lens,OI,ARI,Acc1,Acc2,Acc3,MSE1,MSE2,MSE3,AccMD1,AccMD2,AccMD3,MSEMD1,MSEMD2,MSEMD3,iter, nsim))
}


for(i in fileName){
  idx <- str_extract_all(i, "\\d+")[[1]][2]
  df <- readRDS(paste0("CoDAOP/CohesiveMCAR_2_NoDiag/",i))
  df_reg <- df[[1]]
  df_sto <- df[[3]]
  df_nocov <- df[[2]]
  df_mean <- df[[4]]
  ...
  op <- rbind( cbind(idx,Rgr = "Reg",dir = c("D","U"), getData(df_reg)),
               ...
               cbind(idx,Rgr = "Mean",dir = c("D","U"), getData(df_mean)))
...
library(RColorBrewer)
c <- expand.grid(unique(S$alphaLin), unique(S$missing), unique(S$pClust))
colnames(c) <- c("alphaLin","missing","pClust")
...
for( i in 1:dim(c)[1]){
  ...
  d <-  df_op %>%
    filter(pClust == c$pClust[i],  #alpha == c$alpha[i] ,
           alphaLin == c$alphaLin[i], missing == c$missing[i], dir == "D") 
  if(dim(d)[1] > 0 ){
    gbg <- rbind(gbg, c[i,])
    p[[1]] <- d %>%
      ggplot()+
      geom_boxplot(aes( y = OI, x = lambda, color = Rgr))+ ylim(0, 1)+
      #ggtitle(title)+ 
      ylab("Omega Index") + xlab("lambda")+ scale_color_brewer(palette = "Set1")+
      theme( legend.position='none', panel.background = element_blank())+#theme_minimal()+
      facet_grid( vars(pClustOL), vars(alpha)) 
    
    p[[2]] <- d %>%
      ggplot()+
      geom_boxplot(aes( y = ARI, x = lambda, color = Rgr))+ ylim(0, 1)+
      #ggtitle(title)+ 
    ...
    #   #ggtitle(title)+ 
    #   ylab("Average MSE") + xlab("Regression type")+ scale_color_brewer(palette = "Set2")+
    #   theme( legend.position='none', panel.background = element_blank())+#theme_minimal()+
    #   facet_grid( vars(dir),vars(pClustOL))
    ...
    p[[3]] <- d %>%
      ...
      #ggtitle(title)+ 
      ylab("Average MSE for missing data") + xlab("Regression type")+ scale_color_brewer(palette = "Set2")+
      theme( legend.position='bottom', panel.background = element_blank())+#theme_minimal()+
      facet_grid( vars(pClustOL), vars(alpha))
    ...
    p[[4]] <- d %>%
      ...
      theme( legend.position='bottom', panel.background = element_blank())+#theme_minimal()+
      facet_grid( vars(lambda), vars(alpha))
...
```{r, fig.width=12, fig.height=7}
c <- expand.grid(unique(S$alpha), unique(S$alphaLin), unique(S$missing), unique(S$pClust), unique(S$pClustOL))
colnames(c) <- c("alpha","alphaLin","missing","pClust","pClustOL")
for( i in 1:dim(c)[1]){
  p <- list()
  title <- paste0(c$pClust[i]," clust with and alphaLin ", c$alphaLin[i], " alpha = ", c$alpha[i]," overlap type ", c$pClustOL[i])
  d <-  df_op %>%
    filter(alpha == c$alpha[i] ,pClust == c$pClust[i],alphaLin == c$alphaLin[i], pClustOL == c$pClustOL[i], dir == "D")
  if(nrow(d)>0){
print(d %>%
  ggplot()+
  geom_boxplot(aes(x = Rgr, y = OI, color = factor(missing)))+
    ggtitle(title)+ theme( legend.position='bottom', panel.background = element_blank())+
  facet_grid(vars(dir), vars(lambda)))  
    
    print(d %>%
  ggplot()+
  geom_boxplot(aes(x = Rgr, y = MSEavg, color = factor(missing)))+
    ggtitle(title)+ theme( legend.position='bottom', panel.background = element_blank())+
  facet_grid(vars(dir), vars(lambda)))  
  }
}
...
```{r, fig.width=12, fig.height=7}
#getwd()
simOP <- as.data.frame(readRDS("../MetricsDataSim.rds"))
OrigOP <- as.data.frame(readRDS("../MetricsDataOrig.rds"))

param <- "InitParamMiss_Coh_MCAR_LASSO_Cont_2.rds"
param_c <- "InitParamMiss_Coh_MCAR_LASSO_Cont_2_comp.rds"

S <- as.data.frame(readRDS(param_c))

S$missing <- rep(c(0,5,10,25,45), each =9)
S$idx <- 1:dim(S)[1]
simOP$idx <- as.numeric(simOP$idx)
simOP <- left_join(simOP, S, by = join_by(idx == idx))

dfOP <- df_op %>% select(idx, Rgr, iter, dir, OI, ARI, MSEavgMD)

dfOP <- inner_join(simOP, dfOP, by = c('idx' = 'idx', 'type' = 'Rgr', 'grp' = 'iter', 'Dir' = 'dir'))
dfOP$pClust <- as.character(dfOP$pClust)
dfOP$pClustOL <- as.character(dfOP$pClustOL)
dfOP$Conductance <- as.numeric(dfOP$Conductance)
dfOP$modHP <- as.numeric(dfOP$modHP)
dfOP$Qov <- as.numeric(dfOP$Qov)
c <- as.data.frame(expand.grid(unique(S$pClust), unique(S$pClustOL),unique(S$alpha),unique(S$lambda), stringsAsFactors = FALSE))
colnames(c) <- c("pClust", "pClustOL","alpha","lambda")
dfOPn <- dfOP %>% filter(pClust == "equal")
p <- list()
for(i in 1:dim(c)[1]){
  title <- paste0(c$pClust[i], " cluster with ", c$pClustOL[i], " overlap, alpha= ",c$alpha[i], " , lambda", c$lambda[i] )
  d <- dfOPn %>%
    filter(pClust == c$pClust[i], pClustOL == c$pClustOL[i], alpha == c$alpha[i], lambda == c$lambda[i])
  if(dim(d)[1] > 0 ){
    p[[1]] <-(d %>%
  ggplot()+
  geom_point(aes(x = Conductance, y = OI, color = type ), alpha =0.5)+
    theme( legend.position='none', panel.background = element_blank())+
  facet_grid(vars(Dir), vars(missing)))
    
    p[[2]] <- (d %>%
  ggplot()+
  geom_point(aes(x = modHP, y = OI, color = type ), alpha =0.5)+
    theme( legend.position='none', panel.background = element_blank())+
  facet_grid(vars(Dir), vars(missing)))
    
    p[[3]]  <-(d %>%
  ggplot()+
  geom_point(aes(x = Qov, y = OI, color = type ), alpha =0.5)+
...
```{r}
## Network metrics for original graphs
OrigOP$grphId <- rep(1:(dim(OrigOP)/5), each = 5)


```


## Code for when we get final results only
