...
propCorClust <- opLD %>%
  ...
  dplyr::summarise(propImputed = sum(sameClust)/n(),
                   propAssigned = sum(whichClust)/n())
#saveRDS(opLD,"HighDefClusters.rds")
colnames(comDetVals) <- c("Iteration","PcntMissing","ARIImputed","ARIOrig")
comDetVals %>%
  ggplot()+
  geom_violin(aes(x =as.factor(PcntMissing), y = ARIImputed, color = as.factor(PcntMissing)))+
  geom_jitter(aes(x =as.factor(PcntMissing), y = ARIImputed, color = as.factor(PcntMissing)), position = position_jitter(width = 0.1, height = 0.1), alpha =0.5)+
  #   geom_point(aes(x =as.factor(PcntMissing), y = ARIImputed, color = as.factor(PcntMissing)), alpha = 0.2)+
  ggtitle("ARI imputed vs Original assignment")+
  #facet_wrap(vars(color = as.factor(PcntMissing)), ncol =2, scales='free')+
  guides(fill = FALSE)+
  theme_minimal()+
  theme(panel.border=element_rect(fill=NA, colour="grey40"))
...
comDetVals %>%
  ggplot()+
  geom_violin(aes(x =as.factor(PcntMissing), y = ARIOrig, color = as.factor(PcntMissing)))+
  ggtitle("ARI  Original vs Regular Spectral W/o cov assignment")+
  #facet_wrap(vars(color = as.factor(PcntMissing)), ncol =2, scales='free')+
  guides(fill = FALSE)+
  theme_minimal()+
  theme(panel.border=element_rect(fill=NA, colour="grey40"))
...
comDetVals %>%
  ggplot()+
  geom_jitter(aes(x =ARIOrig, y = ARIImputed, color = as.factor(PcntMissing)), alpha =0.5)+
  ggtitle("ARI  Original vs ARI Imputed cov assignment")+
  facet_wrap(vars(color = as.factor(PcntMissing)), ncol =2, scales='free')+
  guides(fill = FALSE)+
  theme_minimal()+
  theme(panel.border=element_rect(fill=NA, colour="grey40"))
...
IC %>% 
    select(starts_with("AIC") | iter) %>%
  pivot_longer(cols = !iter) %>%
  ggplot()+
  geom_line(aes(x = iter, y  = value, color = name))

IC %>% 
    select(starts_with("BIC") | iter) %>%
  pivot_longer(cols = !iter) %>%
  ggplot()+
  geom_line(aes(x = iter, y  = value, color = name))
...
library(foreach)
edgeDF$group1 <- unlist(foreach::foreach(i = edgeDF$Cat1) %do% 
                          which(str_detect(i, clusts) == TRUE, arr.ind = TRUE))

edgeDF$group2 <- unlist(foreach::foreach(i = edgeDF$Cat2) %do% 
                          which(str_detect(i, clusts) == TRUE, arr.ind = TRUE))

edgeDF$cg <- 0
edgeDF$cg[unlist(edgeDF$group1) == unlist(edgeDF$group2)]<-  edgeDF$group1[unlist(edgeDF$group1) == unlist(edgeDF$group2)]
...
```{r}
## find the number of edges between node attribute combinations.
