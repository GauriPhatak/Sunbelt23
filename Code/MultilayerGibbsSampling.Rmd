...
IterativeImpute <- function(g.sim,N,coms,niter, k,covMsng){
  ...
  for (i in 1:niter){
    ...
    for(j in 1:length(Midx)){
      ...
      covARI <- rbind(covARI, 
                      cbind(rep(mclust::adjustedRandIndex(
                        vertex_attr(g, name = covMsng$name[j],index = Midx[[j]]),
                        vertex_attr(g.sim, name = covMsng$name[j],index = Midx[[j]])) ,
                        times = length(Midx[[j]])))
                      )
    }
...
for(p in pctMsng){
  covMsng <- data.frame(name = c("LOTR") ,
                        p = p,
                        cat = I(list(cat)))
  op <- rbind(op , IterativeImpute(g.sim, N, coms,niter,k,covMsng))
}
...
pC = c(0.3,0.4,0.3)
## Highly defined clusters
B = c(0.3,0.05,0.3,0.05,0.05,0.2)
## Simulate network
g.sim <- SimNW(pC,k,N,B)
