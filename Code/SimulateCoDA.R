...
SaveCoDASim <- function(simvec, sim, InitParamFile){
        ...
        #for(dir in c("directed", "undirected")){
          ...
          for(dir in c("directed", "undirected")){
            ...
            opf_Regcov[[lvl]] <- CoDA(G,nc,k = c(k_in, k_out),o = c(o_in, o_out),N,alpha,
                                   ...
                                   alphaLL = NULL,test = test,missing = missing, covOrig = orig_Cov, 
  ...
  return(list(opf_Regcovtot, opf_noCovtot,opf_StoRegcovtot,Gtot, GTlogLikcovtot, GTlogLiknoCovtot, FS))
  #return(0)
}
...
lstNst190 <- SaveCoDASim(simvec ,
                       sim ,
                       InitParamFile)
#"/Code/InitParamMiss_Cohesive_Nested_MCAR_B15_LASSO.rds")
df <- lstNst11
ends <- matrix(0, nrow =0, ncol = 8)
for(i in 1:8){
  #G <- df[[4]][[i]]
  F_u <- df[[1]][[i]]$Ffin
  H_u <- df[[1]][[i]]$Hfin
  ...
  #                     edge.arrow.size = 0.1, edge.color = "grey28")
  ends <- rbind(ends, c(tail(df[[1]][[i]]$OmegaIndex,1), tail(df[[3]][[i]]$OmegaIndex,1), tail(df[[2]][[i]]$OmegaIndex,1),
  tail(df[[1]][[i]]$Loglik,1)[1], tail(df[[3]][[i]]$Loglik,1)[1], tail(df[[2]][[i]]$Loglik,1)[1], 
...
colnames(ends) <- c("OI_R","OI_S","OI_N","LL_R","LL_S","LL_N","MSE_R","MSE_S")
ends <- data.frame(ends)
ends$type <- c(rep(c("D","U"), 4))
ends <- data.frame(ends) %>%
  pivot_longer(-c(type))#, values_to = "Values", names_to = "type")

ends %>%
  filter(name %in% c("OI_R","OI_S","OI_N")) %>%
  ggplot()+
  geom_boxplot(aes(y = value, x = name, color = type))

ends %>%
  filter(name %in% c("MSE_R","MSE_S")) %>%
  ggplot() +
  geom_boxplot(aes(x = name, y = value, color = type))
