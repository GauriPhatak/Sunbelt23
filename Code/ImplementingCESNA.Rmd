...
printplots <- function(opf, GTll, EG){
  comp <-  data.frame( maxari = rep(0,dim(EG)[1]), llike =rep(0,dim(EG)[1]),
                       ...
                       OmegaIdx = rep(0,dim(EG)[1]), accuracy  = rep(0,dim(EG)[1]))
...
#3,7,11
simN <- 3
op <- readRDS(paste0("CoDaOP/OutputFile", sim,".rds"))
opf_cov <- op[[1]]
opf_noCov <- op[[2]]
G <- op[[3]]
GTlogLikcov <- op[[4]]
...
```{r}
EGip <- expand.grid(1:6, S[simN,]$alpha, S[simN,]$alphaLL, S[simN,]$nc)
