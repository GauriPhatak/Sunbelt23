...
getwd()
WD <- paste0(getwd(),"/Combined_aggregated_OHA_normalized_2025-10-07.xlsx")
...
## Can include data in sample type "Influent" , "Sewerage" , "Pump Station" , "Stormwater"
#df <- df[df$SampleType %in% c("Influent" , "Sewerage" , "Pump Station" , "Stormwater"),]
...
# Combining the 5 networks and color code them.
...
mults <- simplify_and_colorize(hwy_grph)#which_multiple(hwy_grph)
pdf(file= "HighWayGraph5_WS_SR_RC.pdf")
...
#readxl::read_excel(WD,sheet = "COVID", guess_max = 10000)
...
saveRDS(DirectGrph, "ShortestPathGrph5_WS_SR_RC.rds")
```
...
attr <- left_join(attr, aggr_102 %>% dplyr::select(Location, meanLogCopiesPerL), by=join_by(name == Location))
...
saveRDS(G_FullGrph, "G_FullGrph5_WS_SR_RC.rds")
