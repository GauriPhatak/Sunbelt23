...
for(i in 1:dim(ww_samp)[2]){
  ...
  op_bp[[i]] <- breakpoints(breakpoints(y ~ y_lag1 + y_lag2, h =dim(ww_samp)[2], data = bpIp), breaks = Nbrk)
...
bp_nat %>% ggplot() +
  geom_point(aes(y = interaction(Location), x = Breakpoint, group = Location, color = factor(brkpt))) +
...
LassoVar <- function(df_ip, d=NULL,Nbrk, retFit = FALSE){
