...
df <- df %>% subset(select = -c(St..Helens,Silverton, Siletz.Tribe ,Ontario, Ontario.Prison, Redmond, Port.Orford))
...
LassoVar <- function(df_split, d, retFit = FALSE){
  ...
  for (m in 1:6) {
    ...
    # plot.igraph(fit1$dag, 
    #             edge.arrow.size = .2,
    #             vertex.size = 5,
    #             vertex.color = c("lightblue"),
    #             vertex.frame.color = "blue",
    #             vertex.label= NA)
    # plot.igraph(fit1$ring,
    #             edge.arrow.size = .2,
    #             vertex.size = 5,
    #             vertex.color = c("lightblue"),
    #             vertex.frame.color = "blue",
...
for (var in colnames(nodeImp)[-c(1, length(colnames(nodeImp)))]) {
  ...
  mutate(label = ifelse( max_rank <= 2, City, NA)) %>%#min_rank <= 1 |
...
for(i in 1:6){
  ...
  plot.igraph(simplify(G[[i]]),directed = T,
              main = paste("Time Series: ", i,"Node measure", var),#df_date_split$dt_range[i],#
              edge.arrow.size = 0.2,
              layout = cbind(as.numeric(layout$lat), as.numeric(layout$lon)),
