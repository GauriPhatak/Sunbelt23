...
getwd()
N <- 200
test_nc <- c(3,4,5,6,7,8)
simbic <- matrix(0, nrow =0, ncol = 5)
##directed
dirSim <- readRDS("../4clustersims_op1Dir.rds")
for(i in 1:length(dirSim[[1]])){
  ll <- -1*tail(dirSim[[1]][[i]]$Loglik,1)[1]
  simbic <- rbind(simbic, c("D", test_nc[i],ll, 
                            BIC(ll, test_nc[i], N),
                            as.numeric(tail(dirSim[[1]][[i]]$OmegaIndex,1))
  )
  )
}

##undirected
undirSim <- readRDS("../4clustersims_op1unDir.rds")
for(i in 1:length(dirSim[[1]])){
  ll <- -1*round(tail(undirSim[[1]][[i]]$Loglik,1)[1],2)
  simbic <- rbind(simbic, c("U", test_nc[i],ll, 
                            round(as.numeric(BIC(ll, test_nc[i], N),2)),
                            as.numeric(tail(undirSim[[1]][[i]]$OmegaIndex,1))
  )
  )
}

simbic <- as.data.frame(simbic)
colnames(simbic) <- c("Dir","nc","ll","bic")
simbic  %>%
  ggplot()+
  geom_point(aes(x = nc, y = as.numeric(bic), color = Dir))+
  facet_wrap(~Dir)
```
...
getwd()
N <- 200
test_nc <- rep(c(3,4,5,6,7,8), 2)
simbic <- matrix(0, nrow =0, ncol = 6)
##directed
dirSim <- opf_Regcov#readRDS("../4clustersims_op1Dir.rds")
for(i in 1:length(dirSim)){
  ll <- -1*tail(dirSim[[i]]$Loglik,1)[1]
  simbic <- rbind(simbic, c("D", test_nc[i],ll, 
                           dirSim[[i]]$bic[1],
                            as.numeric(tail(dirSim[[i]]$OmegaIndex,1)),
                            i
  )
  )
}
...
simbic <- as.data.frame(simbic)
colnames(simbic) <- c("Dir","nc","ll","bic","OI","i")
simbic$Dir <- rep(c("D","U"), each = 6)
simbic  %>%
  ggplot()+
  geom_point(aes(x = nc, y = as.numeric(bic), color = Dir))+
  facet_wrap(~Dir)
...
filePath <- "C:/Users/gauph/Documents/StatisticsMS_PhD/Wastewater-Surveillance-OSU/Sunbelt23/Code/4clustersims_comb.rds"
S <- readRDS(filePath)
S$penalty <- as.character(S$penalty)
## Reading names of files
boxFolder <- "C:/Users/gauph/Box/SimulationOutput/CoDAOP/4ClusterBICtest_big/"
files <- list.files(boxFolder)

N <- 200
test_nc <- rep(c(3,4,5,6,7,8), 3)
simbic <- matrix(0, nrow =0, ncol = 14)
nocov <- matrix(0, nrow =0, ncol = 14)
...
for(fn in files){
  ...
  for(bigN in 1:S$bigN[1]){
    #for(nsim in 1:S$Nsim[1]){
      for(dir in c("directed","undirected")){
        for(nc in S$test_nc[[1]]){
        simbic <- rbind(simbic, unlist(c(dir, nc, bigN, 
                                  df_rds[[1]][[i]]$bic, 
                                  df_rds[[1]][[i]]$OmegaIndex,
                                  S$alpha[sIdx],
                                  S$lambda[sIdx],
                                  S$pClustOL[sIdx],
                                  S$penalty[sIdx],
        ...
        if(length(df_rds[[2]]) > 0){
                  nocov <- rbind(nocov, unlist(c(dir, nc, bigN, 
                                  df_rds[[2]][[i]]$bic, 
...
simbic <- as.data.frame(simbic)
colnames(simbic) <- c("Dir","nc","bigN","bic","ll","nc2","Nnodes","Nedges","OI",
                      "alpha","lambda","pClustOL","penalty", "sIdx")

simbic[,2:11] <- as.data.frame(apply(simbic[,2:11],2,as.numeric))
simbic$Dir <- unlist(simbic$Dir)
simbic$penalty <- unlist(simbic$penalty)
simbic$pClustOL <- unlist(simbic$pClustOL)
...
simbic  %>% filter(pClustOL == "NoOL", Dir == "directed") %>%
  ggplot()+
  geom_point(aes(x = as.numeric(bic), y = OI, color = as.factor(nc))) +
  xlim(min(simbic$bic),800000)+
  facet_grid(rows= vars(lambda), cols = vars(alpha), scales = "free_y")

simbic  %>% filter(pClustOL == "2GrpOL1", Dir == "directed") %>%
  ggplot()+
  geom_point(aes(x = as.numeric(bic), y = OI, color = as.factor(nc))) +
  xlim(min(simbic$bic),10000)+
  facet_grid(rows= vars(lambda), cols = vars(alpha), scales = "free_y")

simbic  %>% filter(pClustOL == "NoOL", Dir == "undirected") %>%
  ggplot()+
  geom_point(aes(x = as.numeric(bic), y = OI, color = as.factor(nc))) +
  xlim(min(simbic$bic),100000)+
  facet_grid(rows= vars(lambda), cols = vars(alpha), scales = "free_y")

simbic  %>% filter(pClustOL == "2GrpOL1", Dir == "undirected") %>%
  ggplot()+
  geom_point(aes(x = as.numeric(bic), y = OI, color = as.factor(nc))) +
  xlim(min(simbic$bic),100000)+
  facet_grid(rows= vars(lambda), cols = vars(alpha), scales = "free_y")

simbic %>% filter(Dir == "directed", pClustOL == "NoOL") %>%
ggplot()+
  geom_point(aes(y = as.numeric(bic), x = nc, color = as.factor(bigN)  )) +
  ylim(min(simbic$bic),100000)+
  facet_grid(rows= vars(lambda), cols = vars(alpha), scales = "free_y")

simbic %>% filter(Dir == "directed", pClustOL == "2GrpOL1") %>%
ggplot()+
  geom_point(aes(y = as.numeric(bic), x =nc, color = as.factor(bigN)  )) +
  ylim(min(simbic$bic),100000)+
  facet_grid(rows= vars(lambda), cols = vars(alpha), scales = "free_y")
...
Nnodes <- 45
Nedges <- 123
boxFolder <- "C:/Users/gauph/Box/SimulationOutput/CoDAOP/RealCOVIDData/"
files <- list.files(boxFolder)
NumWeeks <- 253
test_nc <- rep(c(2,3,4,5,6,7,8), each = 4*3*4*3 )
finOP <- matrix(0, nrow = 0, ncol = 16 )
numCom <- readRDS(paste0(boxFolder,"realCOVID_NumberOfCommunities","102",".rds"))
names <- rownames(numCom)
imputedVals <- matrix(0, ncol = 7, nrow = 0)
for(i in c(1:4, 6,7, 9:21, 23:32, 34:39, 41:NumWeeks)){
  finOP_i <- as.data.frame(cbind(readRDS(paste0(boxFolder,"realCOVID_DataOP", i,".rds")), test_nc, i)) 
  ...
  imputedV <- as.data.frame(readRDS(paste0(boxFolder,"ImputedValues",i,".rds")))
