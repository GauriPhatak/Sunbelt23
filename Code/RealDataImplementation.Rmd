...
boxFolder <- "C:/Users/gauph/Box/SimulationOutput/CoDAOP/RealCOVIDData_102_metrics/"
...
for(i in NumWeeks){
  ...
  Weekmetrics <- as.data.frame(cbind( readRDS(paste0(boxFolder,"MetricsPO",i,".rds")), paramGrid ))
}
...
Weekmetrics[,c(1:25,27,30) ] <- apply( Weekmetrics[,c(1:25,27,30)] , 2, as.numeric)
...
Weekmetrics$front <-  fronts$ranks
...
FirstTwoFronts <- Weekmetrics %>% filter(front %in% c(1,2))
```
