...
ct_sbst <- ct$CITYNAME[ct$CITYNAME %in% unique(dat$Location)]
...
aggr_102 <- aggr %>% filter(weekno == 102)
...
attr <- as.data.frame(cbind(as.data.frame(vertex_attr(directGrph)), city_cntr))

attr <- attr[attr$name %in% aggr_102$Location,]
...
```{r}
G_subGrph <- set_vertex_attr(G_subGrph,name = "CopiesPerul", value = sub_attrs$CopiesPerul)
...
op <- CoDA(G, nc, k = c(0, 0), o = c(0, 1), N, alpha,lambda, thresh, nitermax, 
           orig, randomize = FALSE,c(), c("CopiesPerul"), c(), c(), dir,
           alphaLL, test, missing, covOrig,epsilon = 0, impType = "Reg", 
           alphaLin, penalty, seed, covInit, specOP )
